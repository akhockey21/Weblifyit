...
$page = isset($_GET['p']) ? $_GET['p'] : 'step0';
...
switch ($page) {
    ...
    if ($numrows > 0) {
     ...
     while($row = mysql_fetch_assoc($getmodels)) {
         ...
         $modelmetakeyy = $row["model_meta_key"];
        $modelmetavaluee = $row["model_meta_value"];
         /*
			if (isset($_POST['$modelmetakeyy'])) {
				Usermeta::update(Auth::user()->id, '$modelmetakeyy', 1);
                redirect_to('?p=reload');
			} else {
				Usermeta::update(Auth::user()->id, 'landing_page', 0);
     ...
     if (isset($_POST['submit']) && csrf_filter()) {
			if (isset($_POST['landing_page'])) {
                $landing_page1 = $_POST['landing_page'];
				Usermeta::update(Auth::user()->id, 'landing_page', $landing_page1);
                redirect_to('?p=reload');
			} else {
				Usermeta::update(Auth::user()->id, 'landing_page', 0);
                redirect_to('?p=cancelled');
		...
		if (isset($_POST['submit']) && csrf_filter()) {
			...
			if (isset($_POST['favlanding_page_1'])) {
                $favlanding_page_1 = $_POST['favlanding_page_1'];
				Usermeta::update(Auth::user()->id, 'favlanding_page_1', $favlanding_page_1);
			} 
            if (isset($_POST['favlanding_page_2'])) {
                $favlanding_page_2 = $_POST['favlanding_page_2'];
				Usermeta::update(Auth::user()->id, 'favlanding_page_2', $favlanding_page_2);
			} 
            if (isset($_POST['favlanding_page_3'])) {
                $favlanding_page_3 = $_POST['favlanding_page_3'];
				Usermeta::update(Auth::user()->id, 'favlanding_page_3', $favlanding_page_3);
			} 
...
switch ($page) {
    ...
    if ($numrows > 0) {
     ...
     while($row = mysql_fetch_assoc($getmodels)) {
        ...
        <div class="checkbox">
				<label>
					<input type="checkbox" value="1" name="<?php echo $modelmetakeyy ?>" <?php echo empty(Auth::user()->usermodels["$modelmetakeyy"])?'':getmodelvalue(1,$modelmetakey); ?>><?php echo $modelmetavaluee?>
				</label>
			</div>
            <?php
     }
