		...
		<?php if (Auth::check()): ?>
			<form action="addComment" class="ajax-form">
				...
				<div class="comment-avatar">
					<a href="profile.php?u=<?php echo Auth::user()->id ?>" target="_blank"><img src="<?php echo Auth::user()->avatar ?>"></a>
				</div>
		...
		<div class="comment-content">
			...
			<div class="comment-body">
				<div class="comment-header">
					...
					<% if (auth.moderate) { %>
						<span class="label label-default" title="<?php _e('comments.moderator') ?>"><?php _e('comments.mod') ?></span>
					<% } %> 
				...
				<div class="comment-footer">
					...
					<div class="comment-votes" data-comment="<%= id %>">
						<?php if (Auth::check()): ?>
							...
							<a href="javascript:void(0)" class="upvote <%= (auth.vote === 1 ? 'voted' : '') %>" title="<?php _e('comments.upvote') ?>"><span class="glyphicon glyphicon-thumbs-up"></span></a>
							<span class="sep"></span>
							<span class="downvotes" data-votes="<%= downvotes %>"><%= (downvotes-upvotes > 0 ? downvotes-upvotes : '') %></span>
							<a href="javascript:void(0)" class="downvote <%= (auth.vote === 2 ? 'voted' : '') %>" title="<?php _e('comments.downvote') ?>"><span class="glyphicon glyphicon-thumbs-down"></span></a>

						<?php else: ?>
							<span class="upvotes" data-votes="<%= upvotes %>"><%= (upvotes-downvotes > 0 ? upvotes-downvotes : '') %></span>
							<a href="javascript:void(0)" title="<?php _e('comments.logged_vote') ?>"><span class="glyphicon glyphicon-thumbs-up"></span></a>
							<span class="sep"></span>
							<span class="downvotes" data-votes="<%= downvotes %>"><%= (downvotes-upvotes > 0 ? downvotes-upvotes : '') %></span>
							<a href="javascript:void(0)" title="<?php _e('comments.logged_vote') ?>"><span class="glyphicon glyphicon-thumbs-down"></span></a>
						<?php endif ?>
					</div>
					<span class="comment-bullet">&bull;</span>
					-->
