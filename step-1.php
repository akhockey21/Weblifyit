...
switch ($page) {
	...
	case 'profile':
		...
		if (isset($_POST['submit']) && csrf_filter()) {
		    ...
		    foreach (UserFields::get('user', 'type_of_business', 'business_name','first_name', 'last_name', 'phone', 'email') as $key => $field) {
	...
	case 'messages':
		if (isset($_POST['submit']) && csrf_filter()) {
			...
			if (isset($_POST['main.type_of_business1'])) {
				Usermeta::update(Auth::user()->id, 'main.type_of_business1', 1);
			}
