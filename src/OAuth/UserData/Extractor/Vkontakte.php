<?php namespace OAuth\UserData\Extractor;
...
class Vkontakte extends LazyExtractor {
	...
	{
		if (isset($data['photo_400_orig'])) return $data['photo_400_orig'];
		if (isset($data['photo_200_orig'])) return $data['photo_200_orig'];
	...
	protected function locationNormalizer($data)
	{
		...
		if (!empty($data['city']['title'])) return $data['city']['title'];

		if (!empty($data['country']['title'])) return $data['country']['title'];
	...
	protected function birthdayNormalizer($data)
	{
		if (isset($data['bdate'])) {
			...
			if (strlen($pieces[0]) < 2) $pieces[0] = "0".$pieces[0];
			if (strlen($pieces[1]) < 2) $pieces[1] = "0".$pieces[1];
