...
class Register extends Manager {
	...
	public function signup(array $input)
	{
		...
		if ($this->config['captcha']) {
	    	...
	    	$rules['captcha'] = 'required|captcha:'.$input['recaptcha_challenge_field'];
	    }
	 ...
	 * @return bool
	 */
	public function reminder($email, $captcha = '', $captchaChallenge = '')
	{
		$this->errors->clear();
		...
		if ($this->config['captcha']) {
	    	...
	    	$rules['captcha'] = 'required|captcha:'.$captchaChallenge;
	    }
		...
		$this->mailer->send('emails.activation', compact('reminder'), function($message) use($user) {
		    $message->to($user->email);
		    $message->subject($this->translator->trans('emails.activation_subject'));
		});
