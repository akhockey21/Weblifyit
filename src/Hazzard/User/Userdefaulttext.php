...
class Userdefaulttext {
	 ...
	 * @var string
	 */
	protected $table = 'defaultwebtext';
	 ...
	 *
	 * @param  int 		$catId 	User ID.
	 * @param  string 	$metaKey 	Metadata name.
	 * @param  mixed 	$metaValue  Metadata value.
	 * @param  bool 	$unique 	Optional, default is false. Whether the same key should not be added.
	 * @return int|false 			Meta ID on success, false on failure.
	 */
	public function add($catId, $metaKey, $metaValue, $unique = false)
	{
		if (!$this->isNumeric($catId)) return false;

		$metaKey = sanitize_key($metaKey);

		$metaValue = maybe_serialize($metaValue);

		if ($unique && $this->has($catId, $metaKey)) return false;
	 ...
	 */
	public function get($catId, $metaKey = '', $single = false)
	{
		if (!$this->isNumeric($catId)) return false;

		$metaKey = sanitize_key($metaKey);

		$query = $this->newQuery()->where('cat_id', $catId);
	 ...
	 */
	public function update($catId, $metaKey, $metaValue, $prevValue = '')
	{
		if (!$this->isNumeric($catId)) return false;
		...
		if (!$this->has($catId, $metaKey)) {
			return $this->add($catId, $metaKey, $metaValue);
		}

		$query = $this->newQuery()->where('cat_id', $catId)->where('meta_key', $metaKey);
	...
	*/
	public function delete($catId, $metaKey = '', $metaValue = '')
	{
		$metaKey = sanitize_key($metaKey);
	...
	* @return bool
	*/
	protected function has($catId, $metaKey)
	{
		return $this->newQuery()->where('cat_id', $catId)->where('meta_key', $metaKey)->pluck('id');	
	}
