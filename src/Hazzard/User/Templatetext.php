...
class Templatetext {
	 ...
	 */
	public function add($templateId, $metaKey, $metaValue, $unique = false)
	{
		if (!$this->isNumeric($templateId)) return false;

		$metaKey = sanitize_key($metaKey);

		$metaValue = maybe_serialize($metaValue);

		if ($unique && $this->has($templateId, $metaKey)) return false;
	 ...
	 */
	public function get($templateId, $metaKey = '', $single = false)
	{
		if (!$this->isNumeric($templateId)) return false;

		$metaKey = sanitize_key($metaKey);

		$query = $this->newQuery()->where('template_id', $templateId);
	 ...
	 */
	public function update($templateId, $metaKey, $metaValue, $prevValue = '')
	{
		if (!$this->isNumeric($templateId)) return false;
		...
		if (!$this->has($templateId, $metaKey)) {
			return $this->add($templateId, $metaKey, $metaValue);
		}

		$query = $this->newQuery()->where('template_id', $templateId)->where('meta_key', $metaKey);
	...
	*/
	public function delete($templateId, $metaKey = '', $metaValue = '')
	{
		$metaKey = sanitize_key($metaKey);
	...
	* @return bool
	*/
	protected function has($templateId, $metaKey)
	{
		return $this->newQuery()->where('template_id', $templateId)->where('meta_key', $metaKey)->pluck('id');	
	}
